...
- `gold.py`：中心小额金币生成、外围 `static_grid == 2` high batch 和伴随外围小额金币生成；部分公开图的外围 `static_grid == 2` 数量少于旧三图，生成候选按地图静态层实际格子处理，零合法 static2 候选时退化为 high region 外外围普通格上的 combined fallback batch。
