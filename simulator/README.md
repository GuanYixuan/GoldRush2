...
- 决策阶段可以使用当前上帝视角状态作为近似局面；执行阶段必须仍按真实规则逐步结算金币和炸弹。
- 同一轮按 dispatch order 依次决策和执行 NPC；更早 NPC 已拾取的金币、触发的炸弹会影响后续 NPC 的 path score。
...
- `dynamic_reward_div10 = dynamic_pickup_reward / 10`；`dynamic_pickup_reward` 按 path 内每次移动进入金币格时的 `ceil(0.65 * remaining_gold)` 累加，并同步扣减该 path 的局部剩余金币。
...
7. 引入默认机制近似模型。该步骤拆成如下子阶段：
   ...
   - 7.3 `mechanisms.npc`：提供 seeded NPC 策略 approximation，默认实现 M4a path-level softmax。必须只产生不会越界/撞障碍的动作，因为 `rules.movement.apply_npc_step()` 对 NPC 非法动作 fail-fast。验收测试：同 seed 可复现，每个 NPC 每回合 3 个动作，NPC 可与玩家/NPC 重叠但不撞墙、不撞障碍；同一轮按 dispatch order 依次决策和执行，前序 NPC 的拾金/踩炸弹会影响后序 NPC 的 score。
