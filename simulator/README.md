...
- 第一版实现 `mechanisms.npc.M4aNpcPolicy`，作为 simulator 默认 NPC profile。类名保留历史命名；当前默认权重对应 canonical M4e static-pickup+center profile。
...
- 对每条 path 计算 M4e static-pickup+center score 后按 `softmax(score / temperature)` 采样，不做 argmax 硬选择。
