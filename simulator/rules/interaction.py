...
def apply_step_interactions(
    ...
    actor, holder = _resolve_actor(state, movement)
    ...
    pickup = _apply_gold_pickup(state, holder, actor, position, rules)
    if pickup is not None:
        pickups.append(pickup)

    bomb_trigger = _apply_bomb_trigger(state, holder, actor, position, rules)
    if bomb_trigger is not None:
        bomb_triggers.append(bomb_trigger)

    trample = _apply_player_trample(state, holder, actor, position, rules)
...
def _apply_bomb_trigger(
    ...
    if holder is None:
        raise ValueError(f"player bomb trigger has no gold holder: {actor}")
    lost = _ceil_fraction(holder.gold, rules.bomb_loss_numerator, rules.bomb_loss_denominator)
    holder.gold -= lost
...
def _apply_player_trample(
    ...
    if actor.kind != ActorKind.PLAYER_UNIT:
        return None
    if holder is None:
        raise ValueError(f"player trample has no gold holder: {actor}")

    npc_count = _npc_count_at(state, position)
    if npc_count < rules.trample_npc_threshold:
        return None

    penalty = _ceil_fraction(holder.gold, rules.trample_loss_numerator, rules.trample_loss_denominator)
    holder.gold -= penalty
