...
class TerminalWinMarginGoldGainReward:
    ...
    def __call__(self, result: RoundStepResult, *, agent_player_id: int) -> float:
        if self._previous_phi is None or self._previous_gross_gold is None:
            raise SimulatorRuleError("terminal win/margin/gold-gain reward requires reset() before first step")
        ...
        total = terminal_reward + margin_reward + gold_gain_reward
    ...
    def _terminal_win_loss(self, result: RoundStepResult, *, agent_player_id: int) -> float:
        if not result.terminated:
            return 0.0
        if result.game_result is None:
            raise SimulatorRuleError("terminal win/margin/gold-gain reward requires game_result")
        if result.game_result.winner_id == agent_player_id:
            return 1.0
        return -1.0
