...
- 当前 `GoldRushPolicyNetwork` 使用 actor/critic 参数分离。`ppo_update()` 中 policy/entropy 梯度只作用于 actor 路径，value 梯度只作用于 critic 路径；critic 暂时仍吃 feature v1，而非 privileged state。
