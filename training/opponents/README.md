...
这些策略是训练/评估基础件，不追求强度。后续再包装更多 `fast_probing/` 或平台强手行为复刻策略。
...
第一阶段尚未实现 C++ runner。后续若需要接 C++ opponent，优先考虑 `cpp_subprocess` 用于评估/回归，协议采用 line-delimited JSON：
