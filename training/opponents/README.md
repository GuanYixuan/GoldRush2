...
- `fast_probe_v3_bfs`：在 `fast_probe_v3_like` 基础上恢复 BFS；只选择 BFS 可达的最大可见金币堆，并沿 BFS path 行动，到达后复用有限 bounce。无可见金币目标时，中心 fallback 会基于当前 `EpisodeContext` 的静态地图对中心 `5x5` 随机非障碍目标做短期 commit 并走 BFS；若新视野发现可达金币，仍会优先吃金币。该对手定位为较强的中心经济型 scripted opponent。
