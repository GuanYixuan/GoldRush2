...
| 地图行 `2` | 数据推断 | replay 静态地图层中的特殊非障碍格；不可从局中 `GameInput.grid` 直接观测，但离线统计显示其与外围高额 batch 金币生成强相关。 |
...
| `players[].cost` | 官方对应 | 玩家策略本轮决策耗时，原始单位为纳秒；前端按 `ns` / `μs` / `ms` / `s` 格式化展示。 |
...
- `0`: 空地或非静态障碍格。
- `1`: 前端按静态障碍物绘制。
- `2`: replay 静态地图层中的特殊非障碍格；当前前端静态地图绘制逻辑未把它当障碍绘制。
...
- `mechanism/results/gold_generation_observation.md` 基于双视角合并 replay 的金币生成统计进一步显示，外围 `static_map=2` 格具有明显高额 batch 生成特征：事件率约 `0.020`，单次事件均值约 `19..21`，主体金额落在 `16..28`；该特征属于 replay/离线建模层信息，官方局中接口不会把“2号格”作为 `GameInput.grid` 的取值直接提供给策略。
...
- 官方对应：`cost`、`gold`、`vision_spent`、`order`、`units`。
- 平台确认：`id`。

`start.players[]` 和 `end.players[]` 常见字段：

- `id`: 玩家 ID，通常为 `1` 或 `2`。
- `cost`: 玩家策略本轮决策耗时，原始单位为纳秒；前端会格式化为 `ns` / `μs` / `ms` / `s`。
- `gold`: 该玩家毛金币总数。
- `vision_spent`: 视野花费。
- `order`: 玩家内两个角色执行顺序。
- `units`: 角色数组。
...
- 官方平台前端 `http://47.103.127.219/game/?id=49823` 当前 bundle 中，玩家耗时展示函数对 `players[].cost` 执行 `<1000 => ns`、`/1000 => μs`、`/1000/1000 => ms`、`/1000/1000/1000 => s` 的逐级格式化。
- 同一对局 replay 前几轮样本中，`round=0` 的 `players[].cost=4320` 对应前端显示 `4.32μs`，`449695` 对应 `449.69μs`，因此 replay 原始值单位为纳秒。
