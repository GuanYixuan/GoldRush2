...
r_t =
    ...
    + beta_vision_info * vision_info_t
```
...
`vision_info_t` 是用于唤醒视野购买探索的可选辅助项，默认关闭。它的目标不是补贴购买成本，而是给“买视野后确实看到此前难以确认的外围金币”一点训练信号。

每次 `RoundStepEnv.step()` 在非终局时会先结算本轮行动，再为下一轮 observation 生成并应用金币；这些“下一轮 observation 对应的新金币事件”通过 `RoundStepResult.next_round_gold_generated` 暴露给 reward。`TerminalWinMarginGoldGainReward` 维护两个 `17x17` 小表：

- `last_seen_round[row][col]`：agent 最近一次实际看见该格的回合。
- `last_gold_increase_round[row][col]`：该格最近一次被下一轮金币生成事件增加的回合。

若 agent 本轮 `vp > 0`，在下一轮 observation 中计算：
...
- 买视野看到额外环带金币才可能奖励。
- 最近看过且金币没有新增的格子不重复奖励。
- 最近看过空地，但之后该格刷出/增加金币，再通过购买视野看到，会奖励。
...
vision_info_reward_t =
    min(
        ...
        beta_vision_info * min(1, fresh_extra_gold / vision_info_scale)
    )
```
